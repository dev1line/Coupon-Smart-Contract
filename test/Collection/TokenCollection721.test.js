...
describe("TokenERC721:", () => {
  ...
  describe("Deployment:", async () => {
    ...
    it("Check tokenURI: ", async () => {
      ...
      await tokenERC721.mint(user3.address, URI);
  ...
  describe("setTokenURI function:", async () => {
    it("Should revert when invalid admin contract address", async () => {
      await expect(
        tokenERC721.connect(user1).setTokenURI("new_uri.json", 1)
  ...
  describe("setMaxBatch", async () => {
    ...
    it("Should revert Invalid maxBatch", async () => {
      await expect(tokenERC721.setMaxBatch(0)).to.revertedWith(
        "InvalidMaxBatch()"
      );
    });
  ...
  describe("setAdminByFactory", async () => {
    it("Should revert when invalid factory contract address", async () => {
      await expect(
        tokenERC721.connect(user1).setAdminByFactory(user1.address, true)
      ).to.revertedWith("CallerIsNotFactory()");
    });

    it("Should revert Invalid address", async () => {
      await expect(
        tokenERC721.setAdminByFactory(AddressZero, true)
  ...
  describe("setAdmin", async () => {
    it("Should revert when invalid admin contract address", async () => {
      await expect(
        tokenERC721.connect(user1).setAdmin(user1.address, true)
      ).to.revertedWith("Ownable: caller is not the owner");
    });

    it("Should revert Invalid address", async () => {
      await expect(tokenERC721.setAdmin(AddressZero, true)).to.revertedWith(
        "InvalidAddress()"
      );
    });
  ...
  describe("mint function:", async () => {
    it("should revert when caller is not an owner or admin: ", async () => {
      await expect(
        tokenERC721.connect(user1).mint(user3.address, "this_uri")
      ).to.be.revertedWith("CallerIsNotOwnerOrAdmin()");
    });

    it("Should revert when invalid receiver address", async () => {
      await expect(tokenERC721.mint(AddressZero, "this_uri")).to.revertedWith(
        "InvalidAddress()"
      );
    });
    ...
    it("should revert when exceeding total supply: ", async () => {
      ...
      await expect(
        tokenERC721.mint(user3.address, "this_uri")
      ).to.be.revertedWith("ExceedTotalSupply()");
    });

    it("should revert when receiver address equal to zero address: ", async () => {
      await expect(
        tokenERC721.mint(AddressZero, "this_uri")
      ).to.be.revertedWith("InvalidAddress()");
    });

    it("should mint success: ", async () => {
      await tokenERC721.mint(user3.address, "this_uri");
      expect(await tokenERC721.balanceOf(user3.address)).to.equal(1);
      expect(await tokenERC721.getTokenCounter()).to.equal(1);
    });
  });
  ...
  describe("batch mint function:", async () => {
    it("should revert when aller is not an owner or admin: ", async () => {
      await expect(
        tokenERC721.connect(user1).mintBatch(user3.address, 3)
      ).to.be.revertedWith("CallerIsNotOwnerOrAdmin()");
    });

    it("should revert when receiver address equal to zero address: ", async () => {
      await expect(tokenERC721.mintBatch(AddressZero, 3)).to.be.revertedWith(
        "InvalidAddress()"
      );
    });
    ...
    it("should revert when invalid receiver address: ", async () => {
      await expect(
        tokenERC721.mintBatch(tokenERC721.address, 3)
      ).to.be.revertedWith(
    ...
    it("should revert when mint fewer in each batch: ", async () => {
      await expect(tokenERC721.mintBatch(user3.address, 0)).to.be.revertedWith(
        "ExceedMaxMintBatch()"
      );
      ...
      await expect(
        tokenERC721.mintBatch(user3.address, max_batch.add(1))
    ...
    it("should revert when exceeding total supply: ", async () => {
      ...
      await expect(tokenERC721.mintBatch(user3.address, 1)).to.be.revertedWith(
        "ExceedTotalSupply()"
      );
    });

    it("should mint success: ", async () => {
      await tokenERC721.mintBatch(user3.address, 3);
      expect(await tokenERC721.balanceOf(user3.address)).to.equal(3);
    });
  });
  ...
  describe("batch mint with uri function:", async () => {
    it("should revert when aller is not an owner or admin: ", async () => {
      await expect(
        tokenERC721
          .connect(user1)
          .mintBatchWithUri(user3.address, [
            "this_uri",
            "this_uri_1",
            "this_uri_2",
          ])
    ...
    it("should revert when receiver address equal to zero address: ", async () => {
      await expect(
        tokenERC721.mintBatchWithUri(AddressZero, [
          "this_uri",
          "this_uri_1",
          "this_uri_2",
        ])
    ...
    it("should revert when invalid receiver address: ", async () => {
      await expect(
        tokenERC721.mintBatchWithUri(tokenERC721.address, [
          "this_uri",
          "this_uri_1",
          "this_uri_2",
        ])
    ...
    it("should revert when mint fewer in each batch: ", async () => {
      ...
      await expect(
        tokenERC721.mintBatchWithUri(
          user3.address,
          Array(Number(max_batch.add(1))).fill("this_uri")
        )
      ).to.be.revertedWith("ExceedMaxMintBatch()");
    });
    ...
    it("should revert when amount of tokens is exceeded: ", async () => {
      const max_batch = await tokenERC721.maxBatch();
      await tokenERC721.mintBatchWithUri(
        user3.address,
        Array(Number(max_batch)).fill("this_uri")
      );

      await expect(
        tokenERC721.mintBatchWithUri(user3.address, ["this_uri"])
      ).to.be.revertedWith("ExceedTotalSupply()");
    });

    it("should mint success: ", async () => {
      await expect(() =>
        tokenERC721.mintBatchWithUri(user1.address, [
          "this_uri",
          "this_uri_1",
          "this_uri_2",
        ])
  ...
  describe("support interface function:", async () => {
    it("check support interface:", async () => {
      expect(await tokenERC721.supportsInterface("0x5b5e139f")).to.be.true;
    });
  });
});
