...
describe("USD Token:", () => {
  ...
  describe("setController function:", async () => {
    ...
    it("should revert when user address equal to zero address: ", async () => {
      await expect(token.setController(AddressZero, true)).to.be.revertedWith(
        "Invalid address"
      );
    });
  ...
  describe("mint function:", async () => {
    it("should revert when caller not be controller: ", async () => {
      ...
      ).to.be.revertedWith("Only controller can call");
    });

    it("should revert when receiver is zero address: ", async () => {
      await expect(token.mint(AddressZero, 100)).to.be.revertedWith(
        "Invalid address"
      );
    });
    ...
    it("should mint success: ", async () => {
      await expect(() => token.mint(user1.address, 100)).to.changeTokenBalance(
        token,
        user1,
        100
      );
