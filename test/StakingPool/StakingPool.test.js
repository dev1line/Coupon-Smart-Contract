...
describe("Staking Pool:", () => {
  ...
  describe("getUserAmount:", async () => {
    it("should return amount of user: ", async () => {
      ...
      await token
        .connect(user1)
        .approve(staking.address, multiply(500, ONE_ETHER));

      await staking.connect(user1).stake(multiply(500, ONE_ETHER));
      expect(await staking.getUserAmount(user1.address)).to.equal(
        multiply(500, ONE_ETHER)
      );
    });
  });
  ...
  describe("setAcceptableLost:", async () => {
    it("should revet with lost > 100: ", async () => {
      const lost = 101;

      await expect(staking.setAcceptableLost(lost)).to.revertedWith(
        "OverLimit()"
      );
    });
  ...
  describe("stake:", async () => {
    ...
    it("should stake success: ", async () => {
      ...
      await treasury
        .connect(owner)
        .distribute(token.address, user1.address, amount);
    ...
    it("should stake success with more times: ", async () => {
      ...
      const amount = "125000000000000000000";
      await treasury
        .connect(owner)
        .distribute(token.address, user1.address, amount);
  ...
  describe("pendingRewards:", async () => {
    it("should return zero reward: ", async () => {
      const current = await getCurrentTime();
      await staking.setStartTime(current + 10);
      const pendingRewards = await staking.pendingRewards(user1.address);
    ...
    it("should return pending reward: ", async () => {
      ...
      await token.connect(user1).approve(mkpManager.address, ONE_MILLION_ETHER);
      const current = await getCurrentTime();
      const opd = await staking.pendingRewards(user1.address);

      expect(opd).to.equal(0);
      await staking.setStartTime(current + 10);
      await skipTime(1000);
  ...
  describe("claim:", async () => {
    ...
    it("should accept lost 50% first ", async () => {
      ...
      await skipTime(100);
      const pendingRewards = await staking.pendingRewards(user1.address);
      await expect(() => staking.connect(user1).claim()).to.changeTokenBalance(
        token,
        user1,
        +pendingRewards / 2
      );
    ...
    it("should claim success", async () => {
      ...
      expect(
        acceptable(
          pendingRewards.toString(),
          divide(
            multiply(multiply(ONE_ETHER, 0.5), 24 * 60 * 60 + 1),
            ONE_YEAR
          ),
          epsilon
        )
    ...
    it("should claim with no earn", async () => {
      await treasury
        .connect(owner)
        .distribute(token.address, user1.address, parseEther("1000"));
      await treasury
  ...
  describe("unstake:", async () => {
    it("should revert when staking pool for NFT not expired: ", async () => {
      ...
      await expect(
        staking.connect(user1).unstake(ONE_ETHER)
      ).to.be.revertedWith("NotAllowToUnstake()");
    });
    it("should revert when request not finish after 24 hours: ", async () => {
      ...
      await expect(
        staking.connect(user1).unstake(ONE_ETHER)
      ).to.be.revertedWith("MustRequestFirst()");
    });
    it("should revert when connot unstake more than staked amount: ", async () => {
      ...
      await expect(
        staking.connect(user1).unstake(OVER_AMOUNT)
      ).to.be.revertedWith("ExceedAmount()");
    });
    it("should unstake success: ", async () => {
      await treasury
        .connect(owner)
        .distribute(token.address, staking.address, parseEther("1000"));
      await treasury
        .connect(owner)
        .distribute(token.address, user2.address, parseEther("1000"));
      await treasury
      ...
      expect(await token.balanceOf(staking.address)).to.equal(
        subtract(parseEther("1000"), pendingRewards)
      );
    });
  });
  ...
  describe("Check EmergencyWithdraw function: ", async () => {
    it("should deposit success: ", async () => {
      await treasury
        .connect(owner)
        .distribute(token.address, staking.address, 10000);

      await expect(() => staking.emergencyWithdraw()).changeTokenBalance(
        token,
        owner,
        10000
      );
      ...
      expect(await staking.stakedAmount()).to.equal(
        await token.balanceOf(staking.address)
      );
    });

    it("should deposit success with stake token != reward token ", async () => {
      ...
      await treasury
        .connect(owner)
        .distribute(usd.address, staking.address, 10000);
      ...
      // expect((await usd.balanceOf(owner.address)).toNumber()).to.equal(0);
      expect(await staking.stakedAmount()).to.equal(
        await token.balanceOf(staking.address)
      );
    });
  ...
  describe("support interface function:", async () => {
    it("check support interface:", async () => {
      expect(await staking.supportsInterface("0x01ffc9a7")).to.be.true;
    });
  });
});
