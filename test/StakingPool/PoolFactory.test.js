...
describe("Pool Factory:", () => {
  ...
  describe("Deployment:", async () => {
    it("Should revert when invalid admin contract address", async () => {
      await expect(
        upgrades.deployProxy(PoolFactory, [staking.address, AddressZero])
      ).to.revertedWith(`InValidAdminContract("${AddressZero}")`);
      await expect(
        upgrades.deployProxy(PoolFactory, [staking.address, user1.address])
      ).to.reverted;
      await expect(
        upgrades.deployProxy(PoolFactory, [staking.address, treasury.address])
      ).to.reverted;
    });
  ...
  describe("support interface function:", async () => {
    it("check support interface:", async () => {
      expect(await poolFactory.supportsInterface("0x01ffc9a7")).to.be.true;
    });
  });
});
