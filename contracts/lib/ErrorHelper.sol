...
library ErrorHelper {
    ...
    function _checkValidNFTAddress(address _nftAddress) internal {
        ...
        if (
            !NFTHelper.isERC721(_nftAddress) &&
            !NFTHelper.isERC1155(_nftAddress)
        ) {
            revert ErrorHelper.InvalidNftAddress(_nftAddress);
        }
    }
    ...
    function _checkValidOwnerOf1155(
        ...
        if (
            IERC1155Upgradeable(_nftAddress).balanceOf(_to, _tokenId) < _amount
        ) {
    ...
    function _checkCanUpdatePaymentToken(
    ...
    function _checkValidMarketItem(
        uint256 _status,
        uint256 _expected
    ) internal pure {
    ...
    //             IMarketplace(_marketplace).verify(
    //                 _marketItemId,
    //                 _proof,
    //                 msg.sender
    //             ))
    ...
    function _checkMarketItemInSelling(
        uint256 _start,
        uint256 _end
    ) internal view {
        if (!(_start <= block.timestamp && block.timestamp <= _end)) {
    ...
    function _checkPermittedPaymentToken(
        address admin,
    ...
    function _checkValidTimeForCreate(
        uint256 _startTime,
        uint256 _endTime
    ) internal view {
    ...
    function _checkExceedMaxCollection(
        uint256 _maxOwner,
        uint256 _maxOfUser
    ) internal pure {
    ...
    function _checkExceedTotalSupply(
        uint256 _total,
        uint256 _supply
    ) internal pure {
        if (_total > _supply) {
    ...
    function _checkExceedMintTotalSupply(
        uint256 _total,
        uint256 _supply
    ) internal pure {
        if (_total >= _supply) {
    ...
    function _checkTimeForStake(
        uint256 _start,
        uint256 _duration
    ) internal view {
        if (
            !(block.timestamp > _start && _start + _duration > block.timestamp)
        ) {
    ...
    function _checkAmountOfStake(
        IAdmin admin,
        IERC20Upgradeable _paymentToken
    ) internal view {
        if (
            !(address(_paymentToken) != address(0) &&
                admin.isPermittedPaymentToken(_paymentToken))
        ) {
