    ...
    ERC165Upgradeable
{
    ...
    mapping(address => EnumerableSetUpgradeable.AddressSet)
        private _ownerToCollectionAddress;
    ...
    event SetTemplateAddress(
        address indexed templateERC721,
        address indexed templateERC1155
    );
    event SetMaxCollectionOfUser(
    ...
    function create(
        ...
        address _template = _typeNft == NFTHelper.Type.ERC721
            ? address(templateERC721)
            : address(templateERC1155);
        ICollection _collection = ICollection(
            ClonesUpgradeable.cloneDeterministic(address(_template), salt)
        );
        ErrorHelper._checkCloneCollection(address(_collection));
        // store
        CollectionInfo memory newInfo = CollectionInfo(
            _typeNft,
            salt,
            address(_collection),
            admin.owner()
     ...
     */
    function setMaxCollectionOfUser(
        address _user,
        uint256 _newValue
    ) external onlyAdmin notZeroAddress(_user) {
    ...
    function setTemplateAddress(
        address _templateERC721,
        address _templateERC1155
    )
        external
    ...
    {
        return
            interfaceId == type(ICollectionFactory).interfaceId ||
