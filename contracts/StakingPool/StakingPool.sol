    ...
    IStakingPool
{
     ...
     */
    IMarketplace public mkpManager;
    ...
    function unstake(
        ...
        ErrorHelper._checkUnstakeTime(startTime, poolDuration);
        ErrorHelper._checkMustRequested(
            user.lazyUnstake.isRequested,
            user.lazyUnstake.unlockedTime
        );
    ...
    function setRewardRate(
        uint256 _rewardRate
    ) external notZero(rewardRate) onlyAdmin {
        rewardRate = _rewardRate;
    ...
    ) external notZero(_pendingTime) onlyAdmin {
    ...
    function setPoolDuration(
        uint256 _poolDuration
    ) external notZero(poolDuration) onlyAdmin {
        poolDuration = _poolDuration;
     ...
     */
    function getPendingUnstakeTime(
        address user
    ...
    function getPriceFormatUSD(
        ...
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        uint[] memory amountOutMins = IPancakeRouter(pancakeRouter)
    ...
    function getAmountOutWith(uint amount) public view returns (uint) {
        (, int price, , , ) = AggregatorV3Interface(aggregatorProxyBUSD_USD)
            .latestRoundData();
        return ((getPriceFormatUSD(address(stakeToken), busdToken, 1e18) *
            amount) / uint(price * 1e10));
    }
